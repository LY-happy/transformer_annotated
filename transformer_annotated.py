...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        pe = torch.zeros(max_len, d_model)  #->  pe为后续的正弦、余弦编码提供存储空间
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1) #->  position：生成序列中每个位置的索引，用于计算正弦和余弦函数的输入
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):
        ...
        # Q: [batch_size, n_heads, len_q, d_k]
        # -->Q,W_Q是线性层的权重矩阵,self.W_Q(input_Q) 的作用就是将 input_Q 通过一个线性层（权重矩阵 W_Q）进行线性变换，将每个输入向量映射到新的维度
...
class Transformer(nn.Module):
    ...
    def forward(self, enc_inputs, dec_inputs):
        ...
        #dec_logits: [batch_size, tgt_len, tgt_vocab_size] -> [batch_size * tgt_len, tgt_vocab_size]
